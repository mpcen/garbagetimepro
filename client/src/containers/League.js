...
class League extends Component {
	componentDidMount() {
		...
		if(!this.props.loading) {
			this.props.searchLeagueOffice(this.props.location.pathname, this.props.match.params.leagueId);
		}
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if(this.props.match.params.leagueId !== nextProps.match.params.leagueId && !this.props.loading) {
			this.props.searchLeagueOffice(nextProps.location.pathname, nextProps.match.params.leagueId)
		}
	}
	...
	displayLoader() {
		return (
			<div className="league-container">	
				<Dimmer active={true}>
					<Loader size='big'>Fetching Leage Data...</Loader>
				</Dimmer>
			</div>
		);
	}
			...
			<div className="league-container">
				...
				<Switch>
					<Route
						...
						render={props => {
							return (
								<div><h1>Buncha sweet stuff coming to this page soon...</h1></div>
							);
						}}
					/>
					...
					<Route
						path={`${this.props.match.path}/compare/:team1Id/:team2Id`}
						render={props => {
							return (
								<div className="league-compare-container">
									<TeamSelect {...props} />
...
export default connect(mapStateToProps, { searchLeagueOffice })(League);